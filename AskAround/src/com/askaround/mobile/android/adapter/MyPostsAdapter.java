...
public class MyPostsAdapter extends BaseAdapter {
	...
	public MyPostsAdapter(Context ctx) {
		context = ctx;
	...
	@Override
	public int getCount() {
		return DataManager.from(context.getApplicationContext()).getMyPosts() == null ? 0 : DataManager.from(context).getMyPosts().size();
	}

	@Override
	public Object getItem(int position) {
		return DataManager.from(context).getMyPosts().get(position);
	}
